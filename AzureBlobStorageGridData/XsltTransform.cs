...
namespace Simio.Xml
{
    public static class XsltTransform
    {
        ...
        public static TransformResult TransformXmlToDataSet(string rawXml, string stylesheet, Action<string> setFinalXmlResult, out string finalXMLString)
        {
            finalXMLString = String.Empty;
            using (var memStream = new System.IO.MemoryStream())
            {
                ...
                {                    
                    ...
                    using (var xmlWriter = XmlWriter.Create(memStream, new XmlWriterSettings() { ConformanceLevel = ConformanceLevel.Auto }))
                    {
                        ...
                        finalXMLString = xmlWriter.Settings.Encoding.GetString(memStream.GetBuffer(), 0, (int)memStream.Length);
                        string _byteOrderMarkUtf8 = Encoding.UTF8.GetString(Encoding.UTF8.GetPreamble());
                        if (finalXMLString.StartsWith(_byteOrderMarkUtf8))
                        {
                            finalXMLString = finalXMLString.Remove(0, _byteOrderMarkUtf8.Length);
                        }
